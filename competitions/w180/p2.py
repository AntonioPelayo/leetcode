...
Problem: 5357. Design a Stack with Increment Operation
Difficulty: Medium
...
class CustomStack:
  ...
  def pop(self) -> int:
    if len(self.elements) == 0:
      ...
      return self.elements.pop()    # Built in LOL

  def increment(self, k: int, val: int) -> None:
    if len(self.elements) < k:                  # Increment all numbers
      for i in range(len(self.elements)):
        self.elements[i] += val
    else:                                       # Increment first k nums
      for i in range(k):
        self.elements[i] += val
